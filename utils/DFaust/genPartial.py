...
def folderGenPartial(folder):
    ...
    for cat in tqdm(os.listdir(y)):
        f = os.path.join(y, cat)
        partialFolder = os.path.join(x, cat)
        if not os.path.exists(partialFolder):
            os.mkdir(partialFolder)
        for pcd in os.listdir(f):
            genPartialWithKCenters(os.path.join(f, pcd), partialfile=os.path.join(partialFolder, pcd))
