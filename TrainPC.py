...
def train(model, trainLoader, valLoader, args):
    ...
    if args.partial:
        ...
        print("[+] Using FULL POINT CLOUDS AS INPUT")
    ...
    with torch.no_grad():
        for i, (data, labels, names) in enumerate(tqdm(testLoader)):
            ...
            if args.testSave:
                ...
                # save_pcd(data[0][0].detach().cpu().numpy().reshape(-1, 3), os.path.join(os.path.join(args.savePath, args.testOut), "{}_partial.pcd".format(os.path.basename(names[0]))))
                save_pcd(inp[0].detach().cpu().numpy().reshape(-1, 3), os.path.join(os.path.join(args.savePath, args.testOut), "{}_reg.pcd".format(name)))
                # save_pcd(temp[0].reshape(-1, 3), os.path.join(os.path.join(args.savePath, args.testOut), "{}_temp.pcd".format(os.path.basename(names[0]))))
                save_pcd(gt[0].detach().cpu().numpy().reshape(-1, 3), os.path.join(os.path.join(args.savePath, args.testOut), "{}_gt.pcd".format(name)))
...
def get_args():
    ...
    parser.add_argument("--pcnModelPath", type=str, default="pcModel.pth", help="Path to point cloud model")
    parser.add_argument("--clusterPath", type=str, default="clusterModel.pkl", help="Path to cluster model")
    parser.add_argument("--clusterJson", type=str, default=None, help="Path to cluster json")
